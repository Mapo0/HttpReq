...
public class Main {
    ...
    public String getToken() {
        ...
        try {
            ...
            while (m.find()){
                csrfToken=m.group().replace("csrfToken\":\"", "");
            }
    ...
    public URL getReqWithParam() throws IOException {
        ...
        URL url = new URL("https://yandex.ru/maps?text=" + address + "&lang=ru_RU&csrfToken=" + csrfToken);
    ...
    public String getCoordinate() throws IOException {
        ...
        try {
            ...
            Pattern p = Pattern.compile("\"coordinates\":\\[(\\d+.){4}");
            Matcher m = p.matcher(result);
            while (m.find()){
                coordinate=m.group();
            }
